...
comorbidity <- c("T2D", "HTN", "PNEU", "ASTH", "ANX", "DEP") #6
...
for(com in comorbidity){
  disease <- read.csv(file = paste0("/Users/stephen/Desktop/SummerResearch/fromMattew/alopeciaAreata/model_summary_coefficient/", com, 
...
AL.coef <- data.frame(comorbidity_coef, covariates, beta_all, beta_low_all, beta_up_all)
AL.odds <- data.frame(comorbidity_odds, covariates, odds_all, odds_low_all, odds_up_all)

## bar graph
ggplot(AL.coef, aes(fill=covariates, y=beta_all, x=comorbidity_coef)) + 
  geom_bar(position="dodge", stat="identity") +
  coord_flip() +
  theme_bw()
ggsave('/Users/stephen/Desktop/SummerResearch/fromMattew/alopeciaAreata/model_summary_coefficient/coeffocient.png', 
       width = 16, height = 27, dpi = 100)

## Odds figure
ggplot(AL.odds, aes(x=odds_all, y=covariates)) + 
  ...
  theme_bw()
ggsave('/Users/stephen/Desktop/SummerResearch/fromMattew/alopeciaAreata/model_summary_coefficient/odds.png', 
       width = 24, height = 36, dpi = 200)


cova <- c("Ethnic", "Clinic", "Gender_F", "yearBirth", "OB", "Eth_Clin")
for (i in cova){
  AL.new.coeff <- AL.coef[which(grepl(i, AL.coef$covariates)), ]
  AL.new.odds <- AL.odds[which(grepl(i, AL.odds$covariates)), ]
  
  
  ggplot(AL.new.odds, aes(x=reorder(comorbidity_odds, odds_all), y=odds_all)) + 
  ...
  ggsave(paste0('/Users/stephen/Desktop/SummerResearch/fromMattew/alopeciaAreata/model_summary_coefficient/', i, '.odds.png'), 
