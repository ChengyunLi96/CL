...
for(com in comorbidity){
  disease <- read.csv(file = paste0("/Users/stephen/Desktop/SummerResearch/fromMattew/vitiligo/model_summary_coefficient/", com, 
...
VI.coef <- data.frame(comorbidity_coef, covariates, beta_all, beta_low_all, beta_up_all)
VI.odds <- data.frame(comorbidity_odds, covariates, odds_all, odds_low_all, odds_up_all)

## bar graph
ggplot(VI.coef, aes(fill=covariates, y=beta_all, x=comorbidity_coef)) + 
  geom_bar(position="dodge", stat="identity") +
  coord_flip() +
  theme_bw()
ggsave('/Users/stephen/Desktop/SummerResearch/fromMattew/vitiligo/model_summary_coefficient/coeffocient.png', 
       width = 16, height = 27, dpi = 100)

## Odds figure
ggplot(VI.odds, aes(x=odds_all, y=covariates)) + 
  ...
  theme_bw()
ggsave('/Users/stephen/Desktop/SummerResearch/fromMattew/vitiligo/model_summary_coefficient/odds.png', 
       width = 24, height = 36, dpi = 200)


cova <- c("Ethnic", "Clinic", "Gender_F", "yearBirth", "OB", "Eth_Clin")
for (i in cova){
  VI.new.coeff <- VI.coef[which(grepl(i, VI.coef$covariates)), ]
  VI.new.odds <- VI.odds[which(grepl(i, VI.odds$covariates)), ]
  
  
  ggplot(VI.new.odds, aes(x=reorder(comorbidity_odds, odds_all), y=odds_all)) + 
  ...
  ggsave(paste0('/Users/stephen/Desktop/SummerResearch/fromMattew/vitiligo/model_summary_coefficient/', i, '.odds.png'), 
