...
for(com in comorbidity){
  disease <- read.csv(file = paste0("/Users/stephen/Desktop/SummerResearch/fromMattew/atopicDermatitis/model_summary_coefficient/", com, 
...
AD.coef <- data.frame(comorbidity_coef, covariates, beta_all, beta_low_all, beta_up_all)
AD.odds <- data.frame(comorbidity_odds, covariates, odds_all, odds_low_all, odds_up_all)

## bar graph
ggplot(AD.coef, aes(fill=covariates, y=beta_all, x=comorbidity_coef)) + 
  geom_bar(position="dodge", stat="identity") +
  coord_flip() +
  theme_bw()
ggsave('/Users/stephen/Desktop/SummerResearch/fromMattew/atopicDermatitis/model_summary_coefficient/coeffocient.png', 
       width = 16, height = 27, dpi = 100)

## Odds figure
ggplot(AD.odds, aes(x=odds_all, y=covariates)) + 
  ...
  theme_bw()
ggsave('/Users/stephen/Desktop/SummerResearch/fromMattew/atopicDermatitis/model_summary_coefficient/odds.png', 
       width = 24, height = 36, dpi = 200)


cova <- c("Ethnic", "Clinic", "Gender_F", "yearBirth", "OB", "Eth_Clin")
for (i in cova){
  AD.new.coeff <- AD.coef[which(grepl(i, AD.coef$covariates)), ]
  AD.new.odds <- AD.odds[which(grepl(i, AD.odds$covariates)), ]
  
  
  ggplot(AD.new.odds, aes(x=reorder(comorbidity_odds, odds_all), y=odds_all)) + 
  ...
  ggsave(paste0('/Users/stephen/Desktop/SummerResearch/fromMattew/atopicDermatitis/model_summary_coefficient/', i, '.odds.png'), 
