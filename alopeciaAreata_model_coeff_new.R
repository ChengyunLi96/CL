load("/nfs/turbo/umms-alextsoi/lichengyun/EHR_data/fromMatthew/RData/AL_new_model.RData")
...
##comorbidities with good sample size (AL). 
comorbidity <- c("T2D", "HTN", "PNEU", "ASTH", "ANX", "DEP") #6

for (com in comorbidity){
  glm.result <- get(paste0("glm.AL.", com))
  table <- coeff_odds(glm.result)
  write.csv(table, file = paste0("/nfs/turbo/umms-alextsoi/lichengyun/EHR_data/fromMatthew/alopeciaAreata/model_summary_coefficient/", 
                                 com, "_coeff.csv"))
