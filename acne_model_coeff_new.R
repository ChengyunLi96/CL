load("/nfs/turbo/umms-alextsoi/lichengyun/EHR_data/fromMatthew/RData/AC_new_model.RData")
...
##comorbidities with good sample size (AC). 
comorbidity <- c("T2D", "HTN", "RA", "IBD", "T1D", "CKD", "GOUT", "OST", "ATF", "PNEU", "HIV", "ASTH", "ANX", "DEP") #13

for (com in comorbidity){
  glm.result <- get(paste0("glm.AC.", com))
  table <- coeff_odds(glm.result)
  write.csv(table, file = paste0("/nfs/turbo/umms-alextsoi/lichengyun/EHR_data/fromMatthew/acne/model_summary_coefficient/", 
                                 com, "_coeff.csv"))
